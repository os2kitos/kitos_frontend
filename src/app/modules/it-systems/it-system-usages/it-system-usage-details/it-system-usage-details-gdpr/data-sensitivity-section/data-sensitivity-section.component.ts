...
export class DataSensitivitySectionComponent extends BaseComponent implements OnInit {
  ...
  public patchDataSensitivityLevels(valueChange?: ValidatedValueChange<unknown>) {
    if (valueChange && !valueChange.valid) {
      ...
      const controlValues = [
        controls.None.value,
        controls.PersonData.value,
        controls.SensitiveData.value,
        controls.LegalData.value,
      ];
  ...
  public patchSpecificPersonalData(valueChange?: ValidatedValueChange<unknown>) {
    if (valueChange && !valueChange.valid) {
      ...
      const controlValues = [
        controls.CprNumber.value,
        controls.SocialProblems.value,
        controls.OtherPrivateMatters.value,
      ];
      ...
      this.store.dispatch(
        ITSystemUsageActions.patchItSystemUsage({ gdpr: { specificPersonalData: newpecificPersonalData } })
      );
    }
  }
