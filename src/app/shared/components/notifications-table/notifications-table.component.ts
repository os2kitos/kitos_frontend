...
import {
  NOTIFICATIONS_DIALOG_DEFAULT_HEIGHT,
  NOTIFICATIONS_DIALOG_DEFAULT_WIDTH,
  NULL_PLACEHOLDER,
} from '../../constants';
...
export class NotificationsTableComponent extends BaseComponent implements OnInit {
  ...
  public readonly systemUsageRolesOptions$ = this.store.select(selectRegularOptionTypes('it-system-usage-roles')).pipe(
    filterNullish(),
  ...
  ngOnInit(): void {
    this.store
      ...
      .pipe(filterNullish())
      .subscribe((organizationUuid) => (this.organizationUuid = organizationUuid));
  ...
  public onDeactivate(notification: APINotificationResponseDTO) {
    this.confirmationService.confirmAction({
      ...
      onConfirm: () => {
        if (notification.uuid && this.organizationUuid)
